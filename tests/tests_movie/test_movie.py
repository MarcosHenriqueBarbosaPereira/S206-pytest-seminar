...
class TestMovie:
    ...
    def test_omdb_api(self, titulo_filme, expected_year):
        ...
        ), f"Falha no status da resposta para {titulo_filme}. Código retornado: {response.status_code}"

        # Validando o ano do filme
        data = response.json()
        assert (
            data["Year"] == expected_year
        ), f"Falha no ano do filme. Esperado: {expected_year}, mas retornado: {data['Year']}"
        ...
        assert (
            poster_response.status_code == 200
        ), f"Falha ao buscar o poster para {titulo_filme}. Código retornado: {poster_response.status_code}"
